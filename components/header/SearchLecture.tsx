...
const SecondFormSchema = z.object({
    qrCode: z.string().min(1, "الرجاء إدخال رمز صحيح").regex(/^[a-zA-Z0-9]+$/, "الرمز يجب أن يحتوي على أحرف وأرقام فقط"),
});
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="w-[650px] max-w-[90%] rounded-[22.317px] p-5 lg:p-20">
                ...
                {isSuccess && (
                    <Form {...secondForm}>
                        <form onSubmit={secondForm.handleSubmit((values) => secondOnSubmit(values))} className="space-y-4">
                            <h1 className="text-[#000] font-bold text-lg sm:text-[22px]">
                                ضع الرمز الخاص بك
                            </h1>
                            <FormField
