...
import { getSons } from "@/services/parent/sons";
...
function ParentSonsTable({ cPage, selectedClass, selectedSubject }: Props) {
  const limit = 10;
  const { data: sons } = useQuery({
    queryKey: ["parent-sons", cPage, selectedClass, selectedSubject],
    queryFn: () => getSons(cPage || "1", limit, selectedSubject, selectedClass),
  });
  if (!sons)
    return (
  ...
  const currentPage = cPage ?? sons.meta.currentPage;
  const totalPages = sons.meta.totalPages;
  const nextPage = sons.meta.nextPage;
  const previousPage = sons.meta.previousPage;

  return (
    <div>
      <DataTable columns={columns} data={sons.data} />
      <div className="flex items-center justify-center gap-2 text-xl font-bold mt-6">
        ...
        <div className="flex items-center gap-[10px]">
          <span className="text-primary">{sons.count} ابناء</span>
          <svg
...
export default ParentSonsTable;
