import { adminAPI, studentAPI } from "../axios";
...
export async function getStudents(
  ...
  const res = await adminAPI.get<GetStudentsResponse>("/students", {
    params: {
      page,
      limit,
      subject,
...
export async function getStudent(id: string) {
  const res = await adminAPI.get<GetSingleStudentResponse>(`/students/${id}`);
  return res.data;
}
...
export async function updateStudent(id: string, data: TUpdateStudent) {
  const res = await adminAPI.patch<GetStudent>(`/students/${id}`, data);
  return res.data;
}
